...
for (i in 1:length(GEChaptersStart)) {
  if (i == length(GEChaptersStart)) {
    GEChaptersEnd[i] <- length(GreatExpectationsRaw)
    next
  }
  GEChaptersEnd[i] <- GEChaptersStart[i + 1] - 2
}

GEChapterText <- character(length(GreatExpectationsRaw$text))
...
GELastWord <- which(text == "her.")
GreatExpectationsLines <- data.frame(chapter = chapter[1:GELastWord], text = text[1:GELastWord])
...
(Comentar si es positivo o negativo)
...
#### Por capítulos
Estudiaremos la evolución del sentimiento a lo largo del libro según capítulos y cada 100 palabras,
