...
knitr::opts_chunk$set(warning = FALSE, message = FALSE, out.width = "55%", dpi = 100, fig.align = "center")
```
...
haremos énfasis en éstos. Tenemos varias características del texto que podemos utilizar para
representar. La más inmediata en que pensamos al considerar el tema de este trabajo es la
**frecuencia de cada palabra**. Este es un tipo de dato posible de adquirir de forma sencilla, y su
visualización es fácilmente entendible a través de un histograma. También se pueden considerar
frecuencias de grupos de palabras, llamados **n-gramas**.
...
Otra opción de visualización aun más compleja son las **redes de co-ocurrencia**. Estos se
...
las redes de co-ocurrencia de algunos personajes, para estudiar hasta qué punto estos pueden
...
importarlos descargándolos directamente desde R con la librería `gutenbergr`, en inglés. El código
usado para realizar esta memoria se encuentra en
...
relevante, aunque es útil. Para empezar, descargamos el libro y seleccionamos las líneas del texto
(eliminamos lo que aparece al principio, el índice y la portada, y lo que aparece al final, así como
el título de los capítulos y las partes del libro). Mientras tanto, vamos etiquetando cada línea con
el capítulo al que pertenece y la parte del libro (que de ahora en adelante llamaremos libro por
simplificar). De esta forma podremos luego realizar un análisis por libros o capítulos.
...
La forma visual de representar esta información es mediante un histograma. Para construir
histogramas utilizaremos siempre la librería `ggplot2`, de las más potentes y conocida por todo el que
...
arbitraria. En la representación aparecen interconectadas por líneas las palabras. De esta forma una
red de co-ocurrencia nos dice las relaciones existentes en un texto entre una palabras y las demás,
...
Construyamos la red de co-ocurrencia del señor Bounderby y de la señora Sparsit, una viuda que un
...
## Visualización de características
### Palabras más frecuentes
Veamos cuáles son las 25 palabras más frecuentes en "Grandes Esperanzas" junto con el contaje
de sus ocurrencias en forma de un histograma,
...
#str(GEWords)
GEWords <- subset(GEWords, GEWords$word != "“i")
#head(GEWords, 25)
```

```{r, echo = FALSE, width = 6, height = 6}
GEWords %>% filter(n > 250) %>% mutate(word = reorder(word, n)) %>%
  ggplot(aes(word, n)) + geom_col() + coord_flip() + labs(title = "Palabras más comunes en Grandes Esperanzas")
```

y veamos ahora que aspecto tiene una nube de palabras con este conjunto de datos,
...
y podemos observar claramente cómo el nombre de los personajes, así como expresiones utilizadas
para referirse a ellos ("dear boy") son los bigramas más comunes, sugiriendo una historia
firmemente centrada en describirlos a ellos y sus acciones.
...
Para deshacernos del ruido y cuantificar adecuadadmente el sentimiento, haría falta analizar el
contexto en el que ha sido mencionada esta palabra, cosa que queda fuera de los objetivos de este
trabajo. Se puede hacer de una forma algo tosca y algo manual (pero no tiene porqué ser poco
efectiva) restando la ocurrencia de "miss" a la ocurrencia de bigramas como "miss havisham", pero
claro, hemos de extraer dichos bigramas y ver qué implican, en lugar de dejar que esto se haga
automáticamente.

Esto nos permite observar una de las ventajas de semejante visualización; nos ha permitido ver
una anomalía en el procesado del texto, lo cual resulta de suma importancia al hacer análisis más
profundos para refinar el código y/o las técnicas utilizadas.
...
n <- filter(.data = ChaptersSentiment, sentiment == "negative")
p <- filter(.data = ChaptersSentiment, sentiment == "positive")
...
```{r, echo = FALSE}
# Determinamos el término principal a partir del cual las co-ocurrencias se miden
coocTerm <- "pip"
```
...
entretenido, dado que poseen un lenguaje algo más rico y detallado que algo como reseñas de productos,
especialmente en el caso de Dickens.
