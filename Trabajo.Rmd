...
```{r}
HardTimesBooksBigrams %>% filter(book == 3) %>% mutate(bigram = reorder(bigram, n)) %>% slice(1:19) %>% ggplot(aes(x = n, y = bigram)) + geom_col() + labs(title = "book 3")
```
...
Primero, seleccionamos el diccionario,
...
y ahora veamos las 25 palabras positivas y negativas que más aparecen en el libro.
...
un tipo u otro. Para evaluar el sentimiento, consideramos éste simplemente como las veces que
aparece cada palabra, y si esta palabra es negativa le cambiaremos el signo. A continuación,
representaremos este histograma.
...
este nivel de detalle. Como tendencia más o menos general podemos decir que el sentimiento va
...
Lo trataremos como un solo libro. La obra en sí comienza más abajo de lo que descargamos de
Gutenberg, dado que se incluye una lista de capítulos aquí, así que hemos de quitar estas líneas
para poder clasificar las líneas por capítulos adecuadamente.
...
GEChaptersStart <- vector("list", length = length(GEBooksStart))
GEChaptersEnd <- vector("list", length = length(GEBooksStart))

# Última palabra del libros
e <- which(text == "her.")
...
leídas según los espacios (" ") que se encuentren.
...
## Visualización de características
### Palabras más frecuentes
Veamos las 25 palabras más frecuentes en "Grandes Esperanzas".
...
y ahora representamos gráficamente 
...
GreatExpectationsBigrams %>% mutate(bigram = reorder(bigram, n)) %>% slice(1:19) %>% ggplot(aes(x = n, y = bigram)) + geom_col() +   labs(title = "book 1")
```
...
palabra es negativa) podemos calcularlo a intervalos a lo largo del libro y estudiar su evolución.
Este es un ejercicio muy recurrente en las fuentes bibliográficas [1] y [2]. Para empezar, veamos el
sentimiento de cada capítulo del libro con un histograma.

```{r}
GreatExpectationsSentiment <- inner_join(x = GreatExpectationsWords, y = sentiments)

GEChapterSentiment <- count(x = GreatExpectationsSentiment, book, sentiment)
