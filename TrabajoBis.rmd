...
# principio y final de los libros
...
e <- which(text == "cold.") # última línea del libro
...
signos de puntuación. También hemos de recordar el uso muy común en ingles del apóstrofe "’". Este
caracter será mejor no eliminarlo, para conservar como una palabras por ejemplo "don’t".
...
utilizaremos el comando `tolower` para pasar todas las letras de las palabras a minúsculas.
...
más a lo largo del texto (repeticiones de las palabras: variable `n`). Para facilitar el trabajo
recurrimos a la librería `dplyr`. Esta contiene un montón de funciones para el contaje, filtrado y
modificación de estructuras de datos. La librería `tidytext` nos proporciona un lexicon con palabras
vacías o *stop words* (palabras como *the* o *and*). Con estas dos librerías podemos contar las
apariciones de cada palabra en el texto y excluir aquellas que no aporten significado al mismo.
Veamos las 25 palabras más frecuentes en Tiempos Difíciles.
...
La forma visual de representar esta información es mediante un histograma. Para contruir las
gráficas utilizaremos siempre la librería `ggplot2`, de las más potentes y conocida por todo el que
utilice RStudio. Para hacer la representación filtramos primero las palabras que aparecen más veces
que un umbral y luego las ordenamos según sus apariciones, variable `n`.

```{r}
WORDS %>% filter(n > 180) %>% mutate(word = reorder(word, n)) %>%
  ggplot(aes(word, n)) + geom_col() + coord_flip() + labs(title = "most common words in Hard Times")
```

Esta información también la podemos representar mediante una nube de palabras. Estas son
...
HardTimesWords %>%
  ...
  count(word) %>%
  {wordcloud(.$word, .$n, max.words = 100, scale = c(3,.5))}
```
...
HardTimesBooksBigrams %>% filter(book == 1) %>% mutate(bigram = reorder(bigram, n)) %>% slice(1:19) %>% ggplot(aes(x = n, y = bigram)) + geom_col() +   labs(title = "book 1")
```
...
```{r}
HardTimesBooksBigrams %>% filter(book == 2) %>% mutate(bigram = reorder(bigram, n)) %>% slice(1:19) %>% ggplot(aes(x = n, y = bigram)) + geom_col() +   labs(title = "book 2")
```
...
```{r}
HardTimesBooksBigrams %>% filter(book == 3) %>% mutate(bigram = reorder(bigram, n))
                      %>% slice(1:19)
...
`tidytext` proporciona un diccionario con información acerca de si una palabras es negativa o
...
en el diccionario. Para contruir una estructura de datos con las palabras del libro positivas y
...
Para visualizar esta información recurrimos una vez más a un histograma, pero con una diferencia con
respecto al anterior, ahora tenemos las palabras clasificadas en dos grupos. Como estamos
interesados en un análisis de sentimiento escogeremos las palabras que aparezcan más veces que
cierto umbral, el mismo para las positivas y las negativas. Así podremos ver si hay más palabras de
un tipo u otro. Para evaluar el sentimiento, consideramos este simplemente como las veces que
aparece cada palabra, y si esta palabra es negativa le cambiaremos el signo. Representamos a
continuación este histograma.
...
(educada, elegante y por decirlo de alguna forma, con mucha ponposidad) exagerada hasta el extremo
sobre todo en ciertos personajes.

La información expuesta anteriormente podemos representarla también con una nube de palabras, en que
...
tratamos por ejemplo de ver cuéles son las palabras negativas que más aparecen tendremos serias
...
Tal y como hemos definido el sentimiento (frecuencia de aparición de las palabras, negativo si la
palabra es negativa) podemos calcularlo a intervalos a lo largo del libro y estudiar su evolución.
Este es un ejercicio muy recurrente en las fuentes bibliográficas [1] y [2]. Para empezar, veamos el
sentimiento de cada parte del libro con un histograma.
...
histograma anterior. Esto es debido a la capacidad limitada de estas técticas de análisis,
recordemos que solo hemos visto si las palabras individualmente tienen una connotación positiva o
negativa. Esto no es suficiente para un libro de literatura con un lenguaje complejo, harían falta
técticas más complicadas que están fuera del alcance y los objetivos de este trabajo.
...
historia contada en el libro haría falta profundizar mucho más en la historia, no llegaremos a hasta
este nivel de detalle. Como tendencia más o mejor general podemos decir que el sentimiento va
...
trágicos, la tendencia general es positiva. Estos detalles vemos que el análisis de sentimiento los
ha detectado más o menos bien.
...
ggplot(Group10LinesSentiment, aes(x = Group100Words, y = sentiment))
...
data(hash_lemmas) # lemma dictionary

data(sw_jockers) # stop words
...
binDTM <- corpus_tokens %>% 
  tokens_remove("") %>%
  dfm() %>% 
  dfm_trim(min_docfreq = minimumFrequency, max_docfreq = 10000000) %>% 
  dfm_weight("boolean")
```
...
```{r}
# Ponemos todos los estadísticos de significancia en un solo data frame
...
Veamos qué nos depara este libro. Está dividido en 59 capítulos, sin estar dividido por libros.
Lo trataremos como un solo libro, sin distinguir entre capítulos. El texto comienza en la línea 31.
...
GreatExpectationsRaw$text <- GreatExpectationsRaw$text[-c(1:(14+59+6))]

GRChaptersStart <- vector("list", length = length(GRBooksStart))
GRChaptersEnd <- vector("list", length = length(GRBooksStart))

for (i in 1:length(GRBooksStart)) {
  s <- which(GRbookBooks == i)[1]
  
  GRChaptersStart[[i]] <- grep(pattern = "CHAPTER", x = textBooks[bookBooks == i]) + s + 3
  GRChaptersEnd[[i]] <- c(ChaptersStart[[i]][2:length(ChaptersStart[[i]])] - 8, BooksEnd[i])
}

GRchapter <- numeric(length(GreatExpectationsRaw$text))
GRtext <- character(length(GreatExpectationsRaw$text))

m <- 1
for (j in 1:length(GRChaptersStart[[i]])) {
  s <- GRChaptersStart[[i]][j]
  e <- GRChaptersEnd[[i]][j]
  
  GRchapter[m:(e - s + m)] <- j
  GRtext[m:(e - s + m)] <- GRtextBooks[s:e]
  
  m <- m + e - s + 1
}

e <- which(text == "her.")
GreatExpectationsLines <- data.frame(book = book[1:e], chapter = chapter[1:e], text = text[1:e])
save(GreatExpectationsLines, file = "data/GreatExpectationsLines.Rdata")
```

Es necesario también dividir el texto por palabras, si queremos contar las veces que aparece cada
una, por ejemplo. Al llevar a cabo este proceso hemos de asegurarnos de eliminar correctamente los
signos de puntuación. También hemos de recordar el uso muy común en ingles del apóstrofe "’". Será
mejor no eliminar este caracter, para conservar palabras como por ejemplo "don’t".
...
```{r}
WordsRaw <- vector("list", length = nrow(GreatExpectationsLines))
Words <- vector("list", length = nrow(GreatExpectationsLines))

book <- numeric(10*nrow(GreatExpectationsLines))
...
for (i in 1:nrow(GreatExpectationsLines)) {
  WordsRaw[[i]] <- strsplit(x = GreatExpectationsLines$text[i], split = "[ .,;—)(?!‘:]")
  Words[[i]] <- WordsRaw[[i]][[1]][!WordsRaw[[i]][[1]] == ""]
  if (length(Words[[i]]) == 0) {
    next
  }
  l <- length(Words[[i]])
  n <- n + l
  book[(n - l + 1):n] <- GreatExpectationsLines$book[i]
...
```{r}
WORDS <- count(GreatExpectationsWords, word, sort = TRUE) %>%
  anti_join(get_stopwords())
...
```{r}
GreatExpectationsBooksBigrams <- GreatExpectationsLines %>% 
  unnest_tokens(bigram, text, token = "ngrams", n = 2) %>% 
  separate(bigram, c("word1", "word2"), sep = " ") %>% 
  filter(!word1 %in% stop_words$word, !word2 %in% stop_words$word) %>% 
  unite(bigram, word1, word2, sep = " ") %>%
...
GreatExpectationsBooksBigrams %>% filter(book == 1) %>% mutate(bigram = reorder(bigram, n)) %>% slice(1:19) %>% ggplot(aes(x = n, y = bigram)) + geom_col() +   labs(title = "book 1")
```
...
```{r}
GreatExpectationsBooksBigrams %>% filter(book == 2) %>% mutate(bigram = reorder(bigram, n)) %>% slice(1:19) %>% ggplot(aes(x = n, y = bigram)) + geom_col() +   labs(title = "book 2")
```
...
```{r}
GreatExpectationsBooksBigrams %>% filter(book == 3) %>% mutate(bigram = reorder(bigram, n))
                      %>% slice(1:19)
                      %>% ggplot(aes(x = n, y = bigram))
                          + geom_col()
                          + labs(title = "book 3")
...
BooksSentiment <- count(x = GreatExpectationsSentiment, book, sentiment)
...
ChaptersSentiment <- list(length(BooksStart))
for (i in 1:length(BooksStart)) {
  ChaptersSentiment[[i]] <- count(x = GreatExpectationssentiment[GreatExpectationssentiment$book == i, ], chapter, sentiment)
}
